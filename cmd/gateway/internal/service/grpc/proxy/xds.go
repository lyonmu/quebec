...
// 		} else {
// 			// 域名解析为 IP 地址
// 			ips, err := net.LookupIP(ip.Host)
// 			if err != nil {
// 				global.Logger.Errorf("域名解析失败 %s: %v", ip.Host, err)
// 				continue
// 			}
// 			// 使用第一个解析到的 IP 地址
// 			if len(ips) > 0 {
...
// 		Name:                 s.Name,
// 		ConnectTimeout:       durationpb.New(time.Duration(global.Cfg.Timeout) * time.Second),
// 		ClusterDiscoveryType: &cluster.Cluster_Type{Type: cluster.Cluster_EDS},
// 		LbPolicy:             common.LoadBalancer[global.Cfg.LbPolicy],
...
// 			PathSpecifier: &route.RouteMatch_Prefix{Prefix: fmt.Sprintf("%s/%s", global.Cfg.RouterPrefix, s.Prefix)},
// 		},
...
// 						ClusterName: common.SentinelGatewayCluster,
// 					},
// 				},
...
// 	if err != nil {
// 		global.Logger.Errorf("failed to marshal HttpConnectionManager: %v", err)
// 		os.Exit(1)
// 	}
...
// 					Address:  "0.0.0.0",
// 					PortSpecifier: &core.SocketAddress_PortValue{
// 						PortValue: uint32(global.Cli.Port),
// 					},
// 				},
// 			},
// 		},
...
// 	// 添加详细日志
// 	global.Logger.Debugf("开始生成 snapshot, 服务数量: %d", len(svcs))

// 	// 1. 生成集群配置 CDS
// 	clusters := make([]types.Resource, 0, len(svcs))
// 	for _, s := range svcs {
// 		cluster := s.MakeCluster()
// 		global.Logger.Debugf("生成集群配置: %s", cluster.Name)
...
// 		global.Logger.Infof("生成端点配置: %s, 实例数量: %d", endpoint.ClusterName, len(s.Instances))
// 		endpoints = append(endpoints, endpoint)
// 	}

// 	// 3. 生成路由配置 RDS
// 	routeCfg := MakeRouteConfig(svcs)
// 	global.Logger.Infof("生成路由配置: %s, 路由数量: %d", routeCfg.Name, len(routeCfg.VirtualHosts[0].Routes))

// 	// 4. 生成监听器配置 LDS
// 	listenerCfg := MakeListener(routeCfg)
// 	global.Logger.Infof("生成监听器配置: %s", listenerCfg.Name)
...
// 	}
// 	// 打印资源统计
// 	global.Logger.Infof("上游服务统计: Clusters=%d, Endpoints=%d, Routes=%d, Listeners=%d",
...
// 	if err != nil {
// 		global.Logger.Errorf("上游服务生成 snapshot 失败: %v", err)
// 		os.Exit(1)
// 	}

// 	// 验证 snapshot
// 	if err := snap.Consistent(); err != nil {
// 		global.Logger.Errorf("上游服务 snapshot 验证失败: %v", err)
// 		os.Exit(1)
// 	}

// 	global.Logger.Info("上游服务 snapshot 生成并验证成功")
// 	return snap
// }
