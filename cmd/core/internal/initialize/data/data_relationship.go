...
func InitDataRelationship(client *ent.Client) error {
	...
	if err != nil {
		global.Logger.Sugar().Error("core_data_relationship表数据初始化失败")
		return err
	}
	if exists {
		global.Logger.Sugar().Info("core_data_relationship表数据无需初始化")
	} else {
		...
		global.Logger.Sugar().Info("core_data_relationship表数据初始化成功")
	}
	return nil
}
